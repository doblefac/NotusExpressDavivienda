...
Namespace General
    ...
    Public Class CargadorArchivoMetasComerciales
...
#Region "Métodos Privados"
        ...
        Private Sub InicializarTablaDatosArchivo()
            ...
            With _dtDatosArchivo
                ...
                .Columns.Add("puntoDeVenta", GetType(String))
        ...
        Private Function EsTitulo(ByVal eRow As ExcelRow) As Boolean
            ...
            Dim arrTitulo As New ArrayList("ESTRATEGIA,PUNTO DE VENTA,PRODUCTO,AÑO,MES,META".Split(","))
        ...
        Private Function FormatoDeFilaEsCorrecto(ByVal eRow As ExcelRow) As Boolean
            ...
            With eRow
                If Not EsNuloOVacio(.Cells("A").Value) AndAlso Not EsNuloOVacio(.Cells("B").Value) _
                    AndAlso Not EsNuloOVacio(.Cells("C").Value) AndAlso Not EsNuloOVacio(.Cells("D").Value) _
                    AndAlso Not EsNuloOVacio(.Cells("E").Value) AndAlso Not EsNuloOVacio(.Cells("F").Value) Then
                    ...
                    If Not oRegExp.IsMatch(.Cells("B").Value.ToString.Trim) Then
                        AdicionarError(eRow.Index + 1, "El campo punto de venta contiene caracteres no válidos.", "PUNTO DE VENTA")
                        resultado = False
                    End If
                    ...
                    If Not RegularExpressions.Regex.IsMatch(.Cells("D").Value.ToString.Trim, "^[0-9]{4,4}$") Then
                        AdicionarError(eRow.Index + 1, "El año proporcionado no es válido. Se espera un valor numérico entero de 4 dígitos", "AÑO")
                        resultado = False
                    End If

                    If Not RegularExpressions.Regex.IsMatch(.Cells("E").Value.ToString.Trim, "^[0-9]{1,2}$") _
                        OrElse CInt(.Cells("E").Value) < 1 OrElse CInt(.Cells("E").Value) > 12 Then
                        AdicionarError(eRow.Index + 1, "El mes proporcionado no es válido. Se espera un valor numérico entre 1 y 12", "MES")
                        resultado = False
                    End If

                    If Not RegularExpressions.Regex.IsMatch(.Cells("F").Value.ToString.Trim, "^[0-9]{1,6}$") Then
                ...
                Else
                    ...
                    If EsNuloOVacio(.Cells("C").Value) Then _
        ...
        Private Sub AdicionarDato(eRow As ExcelRow)
            ...
            With eRow
                ...
                drRow("puntoDeVenta") = .Cells("B").Value.ToString
                drRow("tipoProducto") = .Cells("C").Value.ToString
                drRow("anio") = .Cells("D").Value.ToString
                drRow("mes") = .Cells("E").Value.ToString
                drRow("meta") = .Cells("F").Value.ToString
            End With
            ...
            If resultado.Valor = 0 And Not _hayError Then
                ...
                Try
                    ...
                    With dbManager
                        ...
                        With .BulkCopy
                            .DestinationTableName = "TransitoriaRegistroMetasComerciales"
                            .ColumnMappings.Add("lineaArchivo", "lineaArchivo")
                            .ColumnMappings.Add("estrategia", "estrategia")
                            .ColumnMappings.Add("puntoDeVenta", "puntoDeVenta")
                        ...
                        .llenarDataTable(_dtError, "ValidarDatosRegistroMetasPorArchivo", CommandType.StoredProcedure)
                        ...
                        If _dtError Is Nothing OrElse _dtError.Rows.Count = 0 Then
                            ...
                            .ejecutarNonQuery("RegistrarMetasComercialesPorCargueDeArchivo", CommandType.StoredProcedure)
                            If Long.TryParse(.SqlParametros("@resultado").Value.ToString, resultado.Valor) Then
                                If resultado.Valor = 0 Then
                                    .SqlParametros.Clear()
                                    .SqlParametros.Add("@idUsuario", SqlDbType.Int).Value = idUsuario
                                    .SqlParametros.Add("@identificadorProceso", SqlDbType.UniqueIdentifier).Value = identificadorProceso
                                    .ejecutarNonQuery("LiberarDatosRegistroTransitorioMetas", CommandType.StoredProcedure)
...
#Region "Métodos Compartidos"
        ...
        Public Shared Sub GenerarYDescargarArchivoDeDatosGuia()
            ...
            Dim oExcel As New ExcelFile()
            Dim rutaPlantilla As String = contexto.Server.MapPath("~/Reportes/Plantillas/PlantillaDatosGuiaMetasComerciales.xlsx")
            If File.Exists(rutaPlantilla) Then
                ...
                Dim maestroPdv As New PuntoDeVentaColeccion
                With maestroPdv
                    .IdEstado = 1
                    .CargarDatos()
                End With

                oWs = oExcel.Worksheets("ListaPuntos")
                index = 1
                For Each pdv As PuntoDeVenta In maestroPdv
                    With oWs.Rows(index)
                        .Cells("A").Value = pdv.CodigoOficina
                        .Cells("B").Value = pdv.NombrePdv
                    End With
                    index += 1
                Next

                For Each eCol As ExcelColumn In oWs.Columns
                    eCol.AutoFitAdvanced(1.1)
                Next

                Dim rutaArchivo As String = contexto.Server.MapPath("~/ArchivosCargados/DatosGuiaMetasComerciales_" & contexto.Session("userId") & ".xlsx")

                oExcel.SaveXlsx(rutaArchivo)
                ForzarDescargaDeArchivo(rutaArchivo, Path.GetFileName(rutaPlantilla))
            Else
                Throw New Exception("No se encontró la plantilla de información guía para cargue de metas comerciales. Por favor contracte a IT")
            End If
        End Sub
